...
static INT MSG_(std::string message, int result = NULL, std::string title = "ERROR:")
{
...
bool TCP::Server::start(int port) {
    ...
    if (result == SOCKET_ERROR) {
        MSG_("bind failed with error: ", this->GetLastError());
    ...
    if (result == SOCKET_ERROR) {
        MSG_("listen failed with error: ", this->GetLastError());
        closesocket(_socket_);
        WSACleanup();
        return false;
    }
...
bool TCP::Client::Connect(const std::string& ip, int port)
{
    ...
    if (result != NO_ERROR) {
        MSG_("ioctlsocket failed with error: ", result);
        closesocket(_socket_);
        WSACleanup();
        return false;
    }
    ...
    if (result == SOCKET_ERROR) {
        if (WSAGetLastError() != WSAEWOULDBLOCK) {
            MSG_("connect failed with error: ", this->GetLastError());
            closesocket(_socket_);
            WSACleanup();
            return false;
        }
    ...
    if (result == SOCKET_ERROR) {
        MSG_("select failed with error: ", this->GetLastError());
        closesocket(_socket_);
        WSACleanup();
        return false;
    }
    if (result == 0) {
        MSG_("connect timed out");
        closesocket(_socket_);
        WSACleanup();
        return false;
    }
    ...
    if (error != 0) {
        MSG_("connect failed with error: ", error);
        closesocket(_socket_);
        WSACleanup();
        return false;
    }
